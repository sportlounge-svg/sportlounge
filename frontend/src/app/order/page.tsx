      ...
      <main className="pt-36 md:pt-40 pb-16 min-h-screen relative overflow-hidden smoke-bg">
        ...
        <div className="max-w-5xl mx-auto px-4 relative z-10 animate-fade-in">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold mb-2 text-gold-gradient tracking-wide" style={{ fontFamily: "'Playfair Display', serif" }}>
              Заказ кальяна
            </h1>
          ...
          {!session && (
            <div className="card p-5 mb-8 flex flex-col sm:flex-row items-center justify-between gap-4 max-w-3xl mx-auto" style={{ background: 'rgba(212,165,116,0.02)', borderColor: 'var(--border)' }}>
              <div className="text-center sm:text-left">
                <p className="text-xs font-semibold mb-0.5" style={{ color: 'var(--gold-light)' }}>💼 Личный кабинет</p>
                <p className="text-[10px]" style={{ color: 'var(--text-secondary)' }}>Войдите, чтобы автоматически подставить контакты и сохранять свои миксы</p>
              </div>
              <button onClick={() => router.push('/login')} className="btn-outline btn-sm px-5 py-2 font-bold uppercase tracking-wider rounded-lg cursor-pointer">
                Войти в профиль
              </button>
            </div>
          )}
          ...
          {/* Progress Stepper */}
          <div className="flex flex-wrap items-center justify-center gap-1.5 mb-12 max-w-xl mx-auto font-mono-utility text-[9px]">
            {['Данные', 'Микс', 'Жидкость', 'Проверка'].map((label, i) => (
              <div key={i} className="flex items-center gap-1.5">
                <div className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full font-bold uppercase tracking-wider transition-all duration-300" style={{
                  background: step > i + 1 ? 'rgba(129,199,132,0.04)' : step === i + 1 ? 'rgba(212,165,116,0.05)' : 'rgba(255,255,255,0.01)',
                ...
                {i < 3 && <div className="w-4 h-px" style={{ background: step > i + 1 ? 'var(--success)' : 'var(--border)' }} />}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-2">
              ...
              {step === 1 && (
                <div className="card p-8 shadow-premium animate-fade-in" style={{ border: '1px solid var(--border)' }}>
                  <div className="mb-6">
                    ...
                    <p className="text-xs font-light mt-1" style={{ color: 'var(--text-muted)' }}>Укажите контактную информацию для подачи заказа</p>
                  </div>
              ...
              <div className="flex justify-between mt-10">
                {step > 1 ? (
                  <button onClick={() => setStep(step - 1)} className="btn-outline text-xs font-bold font-mono-utility flex items-center gap-2">
                ...
                {step < 4 ? (
                  <button onClick={() => setStep(step + 1)} disabled={!canProceed()} className="btn-gold text-xs font-bold font-mono-utility flex items-center gap-2"
                  ...
                  <button onClick={handleSubmit} disabled={submitting} className="btn-gold text-xs font-bold font-mono-utility flex items-center gap-2 animate-glow">
            ...
            <div className="lg:col-span-1">
              <div className="card p-6 sticky top-28 shadow-premium animate-fade-in" style={{ border: '1px solid var(--border)' }}>
